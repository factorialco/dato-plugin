 ...
 */
export const checkFormFieldsValidation = (
  formFieldTypes: string[]
  ...
  try {
    ...
    if (duplicateCheck.hasDuplicates) {
      validationErrors.push(
        `Remove duplicated fields - ${duplicateCheck.duplicatedTypes.join(', ')}`
      )
    }
    ...
    if (formFieldTypes.length === 0) {
      ...
      if (missingFields.length > 0) {
        validationErrors.push(
          `Include required fields - ${missingFields.map((field) => REQUIRED_FIELD_TYPES_OBJECT[field as keyof typeof REQUIRED_FIELD_TYPES_OBJECT]).join(', ')}`
        )
      }
    }
