...
describe("checkDuplicateFieldTypes", () => {
  it("reports no duplicates for unique types", () => {
    expect(checkDuplicateFieldTypes(["contact_email", "consent"])).toEqual({
      hasDuplicates: false,
      duplicatedTypes: [],
    });
  });
  ...
  it("reports each repeated type once", () => {
    expect(
      checkDuplicateFieldTypes(["consent", "consent", "consent", "contact_email"])
    ).toEqual({ hasDuplicates: true, duplicatedTypes: ["consent"] });
  });
});

describe("checkFormFieldsValidation", () => {
  it("passes when every required type is present exactly once", async () => {
    await expect(checkFormFieldsValidation(allRequiredTypes)).resolves.toEqual({
      isValid: true,
      validationErrors: [],
    });
  });

  it("lists all required fields when the form is empty", async () => {
    const { isValid, validationErrors } = await checkFormFieldsValidation([]);
  ...
  it("reports missing and duplicated fields together", async () => {
    const { isValid, validationErrors } = await checkFormFieldsValidation([
      "consent",
      "consent",
    ]);
